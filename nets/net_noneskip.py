...
class Net(nn.Module):
    def __init__(self,cfg=None,num_classes=1000, width_mult=1.0, inverted_residual_setting=None, round_nearest=8):
        ...
        block_skip = SkipBlockHalf
        if cfg==None:
          cfg = [ 32, 32, 16, 96, 24, 144, 32, 192, 64 , 384, 96,576, 160,960,320,1280] 
  
        input_channel = 32
        last_channel = cfg[15]  
        # cfgs_skip = [
        #     # input_channel, exp_size, out_channel, kernel, stride, size
        #     [cfg[2], 6, cfg[6], 3, 1, 16],
        #     [cfg[2], 6, cfg[10], 3, 1, 8],
        # ]
        ...
        #     block_2(cfg[2], cfg[4], stride=2, expand_ratio=6,hidden_dim=cfg[3]),
        #     block_2(cfg[4], cfg[6], stride=2, expand_ratio=6,hidden_dim=cfg[5])
        # )

        # self.model3 = nn.Sequential(
        #     block_2(cfg[6], cfg[8], stride=2, expand_ratio=6,hidden_dim=cfg[7]),
        #     block_2(cfg[8], cfg[10], stride=1, expand_ratio=6,hidden_dim=cfg[9])
        # )

        # self.model4 = nn.Sequential(
        #     block_2(cfg[10], cfg[12], stride=2, expand_ratio=6,hidden_dim=cfg[11]),
        #     block_2(cfg[12], cfg[14], stride=1, expand_ratio=6,hidden_dim=cfg[13]),
        #     ConvBNReLU(cfg[14], cfg[15], kernel_size=1)
        # )
